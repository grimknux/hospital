$(document).ready(function(){
	makeAjaxRequest(1);
	$('#cssFeed').addClass('active');
	$('#cssReportLi').addClass('active');
	
	});
function makeAjaxRequest(a){
	if(a==1){
		$.ajax({url:"adminFeedbackResult_exe.php",
		type:"post",
		data:{x:a},
		success:function(b){
			$("div#viewForm").html(b)}
		...
		$.ajax({url:"adminFeedbackResult_exe.php",
		type:"post",
		data:{x:a,cssValType:cssValType,cssDateFrom:cssDateFrom,cssDateTo:cssDateTo,cssDiv:cssDiv},
		success:function(b){
...
var cssValType,cssDateFrom,cssDateTo,cssDiv;
function selectCssType(){
	...
	cssDiv=$("#cssDiv").val();
	var a="";
	if(cssDateFrom==""||cssDateFrom=="0000-00-00"){
	...
	if(cssDiv=="n/a"){
		a+="Please select Internal/External.\n"
	}
	if(a!=""){
		alert(a);
	}else{
